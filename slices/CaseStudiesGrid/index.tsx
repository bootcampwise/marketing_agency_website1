...
export type CaseStudiesGridProps = SliceComponentProps<any>;
                    ...
                    {slice.items.map((item: any, index: number) => {
                        ...
                        return (
                            <div key={index} className="bg-white rounded-xl overflow-hidden shadow-sm">
                                <div className={`h-48 ${bgColorMap[bgColor]} relative flex items-center justify-center`}>
                                    {item.case_study_image?.url ? (
                                        <img
                                            src={item.case_study_image.url}
                                            alt={item.case_study_image.alt || getText(item.case_study_title)}
                                            className="w-full h-full object-cover"
                                        />
                                    ) : (
                                        <span className={`text-xl font-bold ${textColorMap[bgColor]}`}>
