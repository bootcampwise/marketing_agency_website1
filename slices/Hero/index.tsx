        ...
        >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                <div className="max-w-2xl">
                    <h1
                        className="font-sans font-medium mb-6 leading-[1.1] lg:text-[60px] text-4xl -tracking-[0.02em]"
                    >
                        {getText(slice.primary.heading)}
                    </h1>
                    <p
                        className="font-sans text-lg lg:text-[20px] mb-10 text-dark max-w-[500px] leading-relaxed"
                    >
                    ...
                    <div className="relative">
                        <div className="flex animate-scroll-left">
                            ...
                            {slice.items.map((item, index: number) => (
                                item.logo?.url && (
                                    <div key={`logo-1-${index}`} className="relative w-24 h-8 lg:w-32 lg:h-12 mx-6 lg:mx-8 flex-shrink-0 grayscale hover:grayscale-0 opacity-70 hover:opacity-100 transition-all duration-300">
                                        <Image
                                            src={item.logo.url}
                            ...
                            {slice.items.map((item, index: number) => (
                                item.logo?.url && (
                                    <div key={`logo-2-${index}`} className="relative w-24 h-8 lg:w-32 lg:h-12 mx-6 lg:mx-8 flex-shrink-0 grayscale hover:grayscale-0 opacity-70 hover:opacity-100 transition-all duration-300">
                                        <Image
                                            src={item.logo.url}
                                            alt={item.logo.alt || `Client logo ${index + 1}`}
                                            fill
