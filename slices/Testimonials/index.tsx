...
export default function Testimonials({ slice }: TestimonialsProps) {
    ...
    const getClass = (offset: number) => {
        if (!isAnimating) return offset === 0 ? "opacity-100 scale-100" : "opacity-40 scale-95";
    ...
    return (
        <section className="container mx-auto px-4 lg:px-8 py-16">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-8 mb-12">
                <h2
                    className="section-heading bg-primary w-fit px-1 sm:px-2 py-0.5 sm:py-1 rounded text-3xl lg:text-4xl xl:text-5xl"
                >
                    {getText(slice.primary.heading)}
                </h2>

                <p
                    className="text-base lg:text-lg max-w-full lg:max-w-[475px]"
                >
                    {getText(slice.primary.description) || "Hear from Our Satisfied Clients: Read Our Testimonials to Learn More about Our Digital Marketing Services"}
                </p>
            </div>

            {/* Carousel Wrapper */}
            <div
                className="bg-dark rounded-[45px] pt-12 pb-14 text-white overflow-hidden"
            >
                {/* Cards */}
                <div
                    ...
                    <div className="flex gap-3">
                        {items.map((_, i) => (
                            <button
                                key={i}
                                onClick={() => goTo(i)}
                                className={`transition-all duration-300 transform ${i === current ? "text-primary scale-110" : "text-white"
                                    }`}
